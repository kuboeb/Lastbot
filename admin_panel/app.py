...
from admin_panel.routes.simple_ads import register_simple_ads_routes
...
def create_integration():
    ...
    if request.method == 'POST':
        ...
        if integration_type == 'alphacrm':
            ...
            flash('Неизвестный тип интеграции', 'danger')
        ...
        try:
            ...
            conn.commit()
            flash(f'Интеграция "{name}" создана!', 'success')
            return redirect(url_for('integrations'))
...
def test_integration(integration_id):
    ...
    try:
        ...
        # Отправляем
        result = send_to_crm(row[1], row[2], test_data)
...
def test_integration(integration_id):
    ...
    try:
        ...
        if result.get('success'):
            return jsonify({
                'success': True,
                'message': f'Тест успешен! Лид отправлен в {name}'
            })
        else:
            return jsonify({
                'success': False,
...
def delete_integration(integration_id):
    ...
    try:
        ...
        flash('Интеграция успешно удалена', 'success')
        return redirect(url_for('integrations'))
