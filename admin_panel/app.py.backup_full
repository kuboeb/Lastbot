...
from integrations import send_to_crm, send_application_to_crm
import pytz
import os
import requests
import json
...
def create_integration():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn.commit()
            flash(f'Интеграция "{name}" создана успешно!', 'success')
            return redirect(url_for('integrations'))
...
def test_integration(integration_id):
    ...
    try:
        ...
        result = send_to_crm(integration['type'], integration['settings'], test_data)
        ...
        """, (
            integration_id,
            None,  # Нет application_id для теста
